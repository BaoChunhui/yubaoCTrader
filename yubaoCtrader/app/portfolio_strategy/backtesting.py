...
from yubaoCtrader.trader.constant import Direction, Offset, Interval, Status
from yubaoCtrader.trader.database import get_database, BaseDatabase
from yubaoCtrader.trader.object import OrderData, TradeData, BarData
from yubaoCtrader.trader.utility import round_to, extract_vt_symbol, TEMP_DIR
from yubaoCtrader.trader.optimize import (
    OptimizationSetting,
...
class BacktestingEngine:
    ...
    def load_data(self) -> None:
        ...
        for vt_symbol in self.vt_symbols:
            if self.interval == Interval.MINUTE:
                # 尝试加载缓存
                symbol_data = load_cache(vt_symbol, self.interval, self.start, self.end)
                ...
                if not symbol_data:
                    ...
                    while start < self.end:
                        ...
                        for bar in data:
                            # 先缓存在合约字典中
                            symbol_data[(bar.datetime, vt_symbol)] = bar
                    ...
                    # 保存缓存数据文件
                    save_cache(vt_symbol, self.interval, self.start, self.end, symbol_data)

                # 更新到时间戳集合
                for dt, vt_symbol in symbol_data.keys():
                    self.dts.add(dt)

                # 更新到全局字典
                self.history_data.update(symbol_data)
                
                self.output(f"{vt_symbol}历史数据加载完成，数据量：{len(symbol_data)}")
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True) -> dict:
        ...
        if output:
            ...
            self.output("最长回撤天数: \t{}".format(max_drawdown_duration))
            ...
            self.output("日均盈亏：\t{:,.2f}".format(daily_net_pnl))
            self.output("日均手续费：\t{:,.2f}".format(daily_commission))
            self.output("日均滑点：\t{:,.2f}".format(daily_slippage))
            self.output("日均成交金额：\t{:,.2f}".format(daily_turnover))
            self.output("日均成交笔数：\t{}".format(daily_trade_count))

            self.output("日均收益率：\t{:,.2f}%".format(daily_return))
    ...
    def run_bf_optimization(
        ...
        if output:
            for result in results:
                msg: str = "参数：{}, 目标：{}".format(result[0], result[1])
                self.output(msg)
