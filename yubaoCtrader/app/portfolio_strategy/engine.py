...
class StrategyEngine(BaseEngine):
    ...
    def send_order(
        ...
        if not contract:
            self.write_log("委托失败，找不到合约：{}".format(vt_symbol), strategy)
            return ""
    ...
    def add_strategy(
        ...
        if not strategy_class:
            self.write_log("创建策略失败，找不到策略类{}".format(class_name))
            return
    ...
    def _init_strategy(self, strategy_name: str) -> None:
        ...
        if strategy.inited:
            self.write_log("{}已经完成初始化，禁止重复操作".format(strategy_name))
            return

        self.write_log("{}开始执行初始化".format(strategy_name))
        ...
        for vt_symbol in strategy.vt_symbols:
            ...
            if contract:
                ...
                self.write_log("行情订阅失败，找不到合约{}".format(vt_symbol), strategy)
    ...
    def start_strategy(self, strategy_name: str) -> None:
        ...
        if strategy.trading:
            self.write_log("{}已经启动，请勿重复操作".format(strategy_name))
            return
    ...
    def load_strategy_class(self) -> None:
        """加载策略类"""
        path: Path = Path.cwd().joinpath("strategies")
